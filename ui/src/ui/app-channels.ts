...
export async function handleWhatsAppStart(host: ClosedClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: ClosedClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: ClosedClawApp) {
...
function resolveNostrAccountId(host: ClosedClawApp): string {
...
export function handleNostrProfileCancel(host: ClosedClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: ClosedClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: ClosedClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: ClosedClawApp) {
  const state = host.nostrProfileFormState;
