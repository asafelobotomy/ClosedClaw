...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/ClosedClaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/ClosedClaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/ClosedClaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ClosedClaw_CONTROL_UI_BASE_PATH__ = "/ClosedClaw";
    const app = mountApp("/ClosedClaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/ClosedClaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/ClosedClaw/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "ClosedClaw.control.settings.v1",
