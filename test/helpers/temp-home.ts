...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("ClosedClaw_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.ClosedClaw_STATE_DIR = path.join(base, ".ClosedClaw");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "ClosedClaw-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".ClosedClaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
