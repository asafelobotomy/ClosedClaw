...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        ClosedClaw_GATEWAY_TOKEN: inst.gatewayToken,
        ClosedClaw_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          ClosedClaw_GATEWAY_PORT: String(gwA.port),
          ClosedClaw_GATEWAY_TOKEN: gwA.gatewayToken,
          ClosedClaw_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          ClosedClaw_GATEWAY_PORT: String(gwB.port),
          ClosedClaw_GATEWAY_TOKEN: gwB.gatewayToken,
          ClosedClaw_GATEWAY_PASSWORD: "",
        }),
