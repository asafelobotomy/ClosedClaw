...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("ClosedClaw-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("ClosedClaw-bin-sherpa-");
      const modelDir = await makeTempDir("ClosedClaw-sherpa-model-");
      ...
      const cfg: ClosedClawConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("ClosedClaw-bin-whispercpp-");
      const modelDir = await makeTempDir("ClosedClaw-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("ClosedClaw-bin-gemini-");
      ...
      const cfg: ClosedClawConfig = { tools: { media: { image: {} } } };
