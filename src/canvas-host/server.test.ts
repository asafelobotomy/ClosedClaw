...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("ClosedClawCanvasA2UIAction");
    expect(out).toContain("ClosedClawSendUserAction");
  });
  ...
  it("creates a default index.html when missing", async () => {
    ...
    try {
      ...
      expect(html).toContain("ClosedClawSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.ClosedClawA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__ClosedClaw__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("ClosedClaw-a2ui-host");
      expect(html).toContain("ClosedClawCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__ClosedClaw__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("ClosedClawA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
