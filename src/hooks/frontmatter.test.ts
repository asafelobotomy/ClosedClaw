...
`;
    ...
    expect(parsed.ClosedClaw.emoji).toBe("💾");
    expect(parsed.ClosedClaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.ClosedClaw.emoji).toBe("📝");
    expect(parsed.ClosedClaw.events).toEqual(["command"]);
    expect(parsed.ClosedClaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.ClosedClaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"ClosedClaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"ClosedClaw": {"events": ["test"]}}');
  });
...
describe("resolveClosedClawMetadata", () => {
  it("extracts ClosedClaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveClosedClawMetadata(frontmatter);
  ...
  it("returns undefined when ClosedClaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        ClosedClaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with ClosedClaw" },
            { id: "npm", kind: "npm", package: "@ClosedClaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@ClosedClaw/hook");
  });
...
`;
    ...
    const ClosedClaw = resolveClosedClawMetadata(frontmatter);
    expect(ClosedClaw).toBeDefined();
    expect(ClosedClaw?.emoji).toBe("💾");
    expect(ClosedClaw?.events).toEqual(["command:new"]);
    expect(ClosedClaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(ClosedClaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const ClosedClaw = resolveClosedClawMetadata(frontmatter);
    expect(ClosedClaw?.emoji).toBe("disk");
    expect(ClosedClaw?.events).toEqual(["command:new"]);
  });
});
