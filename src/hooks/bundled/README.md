...
This directory contains hooks that ship with ClosedClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.ClosedClaw/ClosedClaw.json`:
...
2. Restart gateway: `pkill -9 -f 'ClosedClaw.*gateway' && pnpm ClosedClaw gateway`
3. Enable the hook: `ClosedClaw hooks enable my-hook`
