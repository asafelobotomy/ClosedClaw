...
homepage: (NOT ASSOCIATED WITH CLOSEDCLAW - Keeping for posterity and future reference) https://docs.OpenClaw.ai/hooks#command-logger
metadata:
  {
    "ClosedClaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.ClosedClaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.ClosedClaw/logs/commands.log ~/.ClosedClaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/ClosedClaw`:
   ```
   /home/username/.ClosedClaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
