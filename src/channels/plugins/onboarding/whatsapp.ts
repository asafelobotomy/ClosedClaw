...
function setWhatsAppDmPolicy(cfg: ClosedClawConfig, dmPolicy: DmPolicy): ClosedClawConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: ClosedClawConfig, allowFrom?: string[]): ClosedClawConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: ClosedClawConfig, selfChatMode: boolean): ClosedClawConfig {
...
async function detectWhatsAppLinked(cfg: ClosedClawConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so ClosedClaw can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for ClosedClaw" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("ClosedClaw channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
