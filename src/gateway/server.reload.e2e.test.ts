...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.ClosedClaw_SKIP_CHANNELS;
    prevSkipGmail = process.env.ClosedClaw_SKIP_GMAIL_WATCHER;
    process.env.ClosedClaw_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.ClosedClaw_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.ClosedClaw_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ClosedClaw_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
