...
import type { ClosedClawConfig, ModelProviderConfig } from "../config/types.js";
import { resolveClosedClawAgentDir } from "../agents/agent-paths.js";
...
import { ensureClosedClawModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.ClosedClaw_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ClosedClaw_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.ClosedClaw_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: ClosedClawConfig;
  agentDir: string;
}): ClosedClawConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<ClosedClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.ClosedClaw_AGENT_DIR,
  ...
  const hostAgentDir = resolveClosedClawAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-live-state-"));
  process.env.ClosedClaw_STATE_DIR = tempStateDir;
  ...
  process.env.ClosedClaw_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.ClosedClaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveClosedClawAgentDir();
  const sanitizedCfg: ClosedClawConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "ClosedClaw live tool probe (local, safe): " +
    ...
    process.env.ClosedClaw_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ClosedClaw_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureClosedClawModelsJson(cfg);
      ...
      const rawModels = process.env.ClosedClaw_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.ClosedClaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
