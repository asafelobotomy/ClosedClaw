...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.ClosedClaw_STATE_DIR = path.join(homeDir, ".ClosedClaw");
  ...
  test(
    ...
    async () => {
      ...
      expect(initial.payload?.triggers).toEqual(["ClosedClaw", "claude", "computer"]);
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".ClosedClaw", "settings", "voicewake.json"), "utf8"),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    ...
    const prevCanvasPort = process.env.ClosedClaw_CANVAS_HOST_PORT;
    process.env.ClosedClaw_GATEWAY_TOKEN = "secret";
    testTailnetIPv4.value = "100.64.0.1";
    testState.gatewayBind = "lan";
    const canvasPort = await getFreePort();
    testState.canvasHostPort = canvasPort;
    process.env.ClosedClaw_CANVAS_HOST_PORT = String(canvasPort);
    ...
    if (prevCanvasPort === undefined) {
      ...
      process.env.ClosedClaw_CANVAS_HOST_PORT = prevCanvasPort;
    }
  });
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.ClosedClaw_CONFIG_PATH;
    if (!configPath) {
      throw new Error("Missing ClosedClaw_CONFIG_PATH");
    }
