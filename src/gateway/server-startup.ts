...
import type { loadClosedClawPlugins } from "../plugins/loader.js";
...
export async function startGatewaySidecars(params: {
  ...
  pluginRegistry: ReturnType<typeof loadClosedClawPlugins>;
  ...
  // Start ClosedClaw browser control server (unless disabled via config).
  ...
  if (!isTruthyEnvValue(process.env.ClosedClaw_SKIP_GMAIL_WATCHER)) {
    try {
  ...
  if (!skipChannels) {
    ...
    params.logChannels.info(
      "skipping channel start (ClosedClaw_SKIP_CHANNELS=1 or ClosedClaw_SKIP_PROVIDERS=1)",
    );
  }
