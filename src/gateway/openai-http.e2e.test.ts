    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "ClosedClaw", messages: [{ role: "user", content: "hi" }] },
          { "x-ClosedClaw-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-ClosedClaw-agent-id": "beta",
            "x-ClosedClaw-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
