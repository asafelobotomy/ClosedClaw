...
export function resolveHooksConfig(cfg: ClosedClawConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, _url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-ClosedClaw-token"] === "string"
      ? req.headers["x-ClosedClaw-token"].trim()
      : "";
  if (headerToken) {
