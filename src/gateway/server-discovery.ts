...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/ClosedClaw/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (ClosedClaw)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.ClosedClaw_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "ClosedClaw");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.ClosedClaw_TAILNET_DNS?.trim();
  ...
  const exec =
    opts?.exec ??
    ((command, args) =>
      runExec(command, args, { timeoutMs: TIMEOUT_TEST_SHORT_MS, maxBuffer: 200_000 }));
  try {
