...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ClosedClaw_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("ClosedClaw_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ClosedClaw_GATEWAY_TOKEN");
    expect(message).not.toContain("ClosedClaw_GATEWAY_PASSWORD");
  });
