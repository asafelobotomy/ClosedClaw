...
beforeAll(() => {
  previousProfile = process.env.ClosedClaw_PROFILE;
  process.env.ClosedClaw_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.ClosedClaw_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/ClosedClaw/pnpm-lock.yaml",
      markerPath: "/tmp/ClosedClaw/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("ClosedClaw status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("ClosedClaw status --all") ||
          l.includes("ClosedClaw --profile isolated status --all") ||
          l.includes("ClosedClaw status --all") ||
          l.includes("ClosedClaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.ClosedClaw_GATEWAY_TOKEN = "abcd1234";
    try {
