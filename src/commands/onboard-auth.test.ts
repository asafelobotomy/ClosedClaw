...
const requireAgentDir = () => {
  const agentDir = process.env.ClosedClaw_AGENT_DIR;
  if (!agentDir) {
    throw new Error("ClosedClaw_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.ClosedClaw_STATE_DIR;
  const previousAgentDir = process.env.ClosedClaw_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.ClosedClaw_AGENT_DIR;
    } else {
      process.env.ClosedClaw_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.ClosedClaw_OAUTH_DIR;
  });

  it("writes auth-profiles.json under ClosedClaw_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-oauth-"));
    process.env.ClosedClaw_STATE_DIR = tempStateDir;
    process.env.ClosedClaw_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ClosedClaw_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ClosedClaw_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-minimax-"));
    process.env.ClosedClaw_STATE_DIR = tempStateDir;
    process.env.ClosedClaw_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ClosedClaw_AGENT_DIR;
