...
export async function statusCommand(
  ...
  runtime.log(theme.heading("ClosedClaw status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("ClosedClaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("ClosedClaw security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("ClosedClaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("ClosedClaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("ClosedClaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("ClosedClaw gateway probe")}`);
  }
}
