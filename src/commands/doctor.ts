...
function resolveMode(cfg: ClosedClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("ClosedClaw doctor");

  const root = await resolveClosedClawPackageRoot({
  ...
  let cfg: ClosedClawConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("ClosedClaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("ClosedClaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("ClosedClaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("ClosedClaw doctor --fix")}" to apply changes.`);
  }
