...
import { readConfigFileSnapshot, type ClosedClawConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: ClosedClawConfig, model: string): ClosedClawConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("ClosedClaw plugins install")}\`.`,
    );
  }
