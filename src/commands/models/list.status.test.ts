...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveClosedClawAgentDir: vi.fn().mockReturnValue("/tmp/ClosedClaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/ClosedClaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/ClosedClaw-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveClosedClawAgentDir: mocks.resolveClosedClawAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveClosedClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/ClosedClaw-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/ClosedClaw-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/ClosedClaw-agent-custom");
