...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveClosedClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("ClosedClaw models auth setup-token")}\` or \`${formatCliCommand("ClosedClaw configure")}\`.`
          : `Run \`${formatCliCommand("ClosedClaw configure")}\` or set an API key env var.`;
