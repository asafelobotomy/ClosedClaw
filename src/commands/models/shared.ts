...
export async function updateConfig(
  mutator: (cfg: ClosedClawConfig) => ClosedClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: ClosedClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ClosedClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("ClosedClaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
