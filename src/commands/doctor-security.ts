    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("ClosedClaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("ClosedClaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("ClosedClaw doctor --fix")} to generate a token`,
