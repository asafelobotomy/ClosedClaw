...
describe("doctor state integrity", () => {
  ...
  it("warns about multiple state dirs when alternate dir is set", async () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "cc-state-extra-"));
    const stateDir = path.join(tempHome, "state-alt");
    const defaultStateDir = path.join(tempHome, ".ClosedClaw");
    fs.mkdirSync(stateDir, { recursive: true, mode: 0o700 });
    fs.mkdirSync(defaultStateDir, { recursive: true, mode: 0o700 });
    ...
    await noteStateIntegrity(cfg, prompter, configPath);

    const messages = note.mock.calls.map((c) => String(c[0])).join("\n");
    expect(messages).toContain("Multiple state directories detected");
    expect(messages).toContain(defaultStateDir);
