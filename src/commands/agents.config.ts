...
export function listAgentEntries(cfg: ClosedClawConfig): AgentEntry[] {
...
function resolveAgentName(cfg: ClosedClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: ClosedClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ClosedClawConfig): AgentSummary[] {
