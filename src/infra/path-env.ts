...
function candidateBinDirs(opts: EnsureClosedClawPathOpts): string[] {
  ...
  // Bundled macOS app: `ClosedClaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "ClosedClaw");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/ClosedClaw` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "ClosedClaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `ClosedClaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureClosedClawCliOnPath(opts: EnsureClosedClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ClosedClaw_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.ClosedClaw_PATH_BOOTSTRAPPED = "1";
