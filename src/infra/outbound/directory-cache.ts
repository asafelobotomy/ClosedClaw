...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ClosedClawConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: ClosedClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: ClosedClawConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: ClosedClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: ClosedClawConfig): void {
