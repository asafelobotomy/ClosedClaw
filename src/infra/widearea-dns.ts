...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ClosedClaw_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*ClosedClaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "ClosedClaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "ClosedClaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "ClosedClaw-gw");
  ...
  records.push(`_ClosedClaw-gw._tcp IN PTR ${instanceLabel}._ClosedClaw-gw._tcp`);
  records.push(`${instanceLabel}._ClosedClaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._ClosedClaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; ClosedClaw-content-hash: ${contentHash}\n${contentBody}`;
}
