...
export function triggerClosedClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.ClosedClaw_SYSTEMD_UNIT,
  ...
  const label =
    process.env.ClosedClaw_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ClosedClaw_PROFILE);
