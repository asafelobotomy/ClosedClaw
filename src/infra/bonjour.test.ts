...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.ClosedClaw_MDNS_HOSTNAME = "test-host";
    process.env.ClosedClaw_MDNS_HOSTNAME = "test-host";
    ...
    expect(gatewayCall?.[0]?.type).toBe("ClosedClaw-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("ClosedClaw (ClosedClaw)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("ClosedClaw");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("ClosedClaw.local");
