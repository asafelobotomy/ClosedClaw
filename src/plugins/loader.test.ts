...
import { loadClosedClawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.ClosedClaw_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `ClosedClaw-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "ClosedClaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.ClosedClaw_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadClosedClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadClosedClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadClosedClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
