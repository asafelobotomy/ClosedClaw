...
describe("browser chrome profile decoration", () => {
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    const userDataDir = await fsp.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-chrome-test-"));
    try {
      decorateClosedClawProfile(userDataDir, { color: DEFAULT_ClosedClaw_BROWSER_COLOR });
      ...
      expect(def.name).toBe(DEFAULT_ClosedClaw_BROWSER_PROFILE_NAME);
      expect(def.shortcut_name).toBe(DEFAULT_ClosedClaw_BROWSER_PROFILE_NAME);
      ...
      const marker = await fsp.readFile(
        path.join(userDataDir, ".ClosedClaw-profile-decorated"),
        "utf-8",
      );
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await fsp.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-chrome-test-"));
    try {
      decorateClosedClawProfile(userDataDir, { color: "lobster-orange" });
      const localState = await readJson(path.join(userDataDir, "Local State"));
      const profile = localState.profile as Record<string, unknown>;
      const infoCache = profile.info_cache as Record<string, unknown>;
      const def = infoCache.Default as Record<string, unknown>;
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    try {
      ...
      expect(profile.name).toBe(DEFAULT_ClosedClaw_BROWSER_PROFILE_NAME);
    } finally {
      await fsp.rm(userDataDir, { recursive: true, force: true });
    }
  });
...
describe("browser chrome helpers", () => {
  ...
  it("picks the first existing Chrome candidate on macOS", () => {
    // macOS detection removed (Linux-only build); falls through to Linux candidates
    const spy = vi
      .spyOn(fs, "existsSync")
      .mockImplementation((p) => String(p) === "/usr/bin/google-chrome");
  ...
  it("picks the first existing Chrome candidate on Windows", () => {
    // Windows detection removed (Linux-only build); falls through to Linux candidates
    const spy = vi
      .spyOn(fs, "existsSync")
      .mockImplementation((p) => String(p) === "/usr/bin/google-chrome");
    const exe = resolveBrowserExecutableForPlatform(
  ...
  it("uses executablePath from config when set", () => {
    ...
    expect(exe?.path).toBe(customPath);
    spy.mockRestore();
  });
  ...
  it("resolves brave as fallback on non-Linux platform", () => {
    // Windows detection removed; falls through to Linux candidates
    const spy = vi
      .spyOn(fs, "existsSync")
      .mockImplementation((p) => String(p) === "/usr/bin/brave-browser");
    const exe = resolveBrowserExecutableForPlatform(
      {} as Parameters<typeof resolveBrowserExecutableForPlatform>[0],
      "win32",
    );
    expect(exe?.kind).toBe("brave");
    expect(exe?.path).toBe("/usr/bin/brave-browser");
    spy.mockRestore();
  });
  ...
  it("stopClosedClawChrome no-ops when process is already killed", async () => {
    const proc = { killed: true, exitCode: null, kill: vi.fn() };
    await stopClosedClawChrome(
      {
        proc,
        cdpPort: 12345,
      } as unknown as Parameters<typeof stopClosedClawChrome>[0],
      10,
    );
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopClosedClawChrome sends SIGTERM and returns once CDP is down", async () => {
