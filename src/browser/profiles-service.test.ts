...
vi.mock("./chrome.js", () => ({
  resolveClosedClawUserDataDir: vi.fn(() => "/tmp/ClosedClaw-test/ClosedClaw/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveClosedClawUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "ClosedClaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveClosedClawUserDataDir).mockReturnValue(userDataDir);
