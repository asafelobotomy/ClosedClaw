import type { BrowserConfig, BrowserProfileConfig, ClosedClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_ClosedClaw_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_ClosedClaw_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "ClosedClaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ClosedClaw_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ClosedClaw_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an ClosedClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for ClosedClaw/ClosedClaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: ClosedClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ClosedClaw_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "ClosedClaw";
