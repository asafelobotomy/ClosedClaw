...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".ClosedClaw-profile-decorated");
}
 ...
 */
export function decorateClosedClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ClosedClaw_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ClosedClaw_BROWSER_COLOR).toUpperCase();
