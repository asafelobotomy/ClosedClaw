...
export class ShortTermMemory<T = any> {
	 ...
	 */
	constructor(
		private readonly defaultTtl: number = AGENTS.MEMORY.SHORT_TERM
			.DEFAULT_TTL_MS,
	...
	set(key: string, value: T, ttl?: number): void {
		...
		if (effectiveTtl > AGENTS.MEMORY.SHORT_TERM.MAX_TTL_MS) {
			throw new Error(
				`TTL must be <= ${AGENTS.MEMORY.SHORT_TERM.MAX_TTL_MS}`,
			);
		}
	...
	get(key: string): T | undefined {
		...
		entry.ttl = Math.min(
			...
			AGENTS.MEMORY.SHORT_TERM.MAX_TTL_MS,
		);
 ...
 */
export function createShortTermMemory<T = any>(
	defaultTtl?: number,
): ShortTermMemory<T> {
