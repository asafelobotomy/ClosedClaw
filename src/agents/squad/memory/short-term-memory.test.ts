...
describe("ShortTermMemory", () => {
	...
	describe("constructor", () => {
		...
		it("throws if TTL < 1", () => {
			expect(() => new ShortTermMemory(0, false)).toThrow(
				"defaultTtl must be >= 1",
			);
		});

		it("throws if TTL > MAX_TTL_MS", () => {
			const tooLarge = AGENTS.MEMORY.SHORT_TERM.MAX_TTL_MS + 1;
			expect(() => new ShortTermMemory(tooLarge, false)).toThrow(
				"defaultTtl must be <=",
			);
		});
	...
	describe("set and get", () => {
		...
		it("throws if custom TTL > MAX_TTL_MS", () => {
			const tooLarge = AGENTS.MEMORY.SHORT_TERM.MAX_TTL_MS + 1;
			expect(() => stm.set("key", "value", tooLarge)).toThrow(
				"TTL must be <=",
			);
		});
