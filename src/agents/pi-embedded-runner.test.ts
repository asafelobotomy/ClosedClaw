...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-embedded-agent-"));
...
const ensureModels = (cfg: ClosedClawConfig) =>
  ensureClosedClawModelsJson(cfg, agentDir) as unknown;
