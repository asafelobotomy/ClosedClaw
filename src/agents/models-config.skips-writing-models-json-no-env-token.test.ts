...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "ClosedClaw-models-" });
}

const MODELS_CONFIG: ClosedClawConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      const previousDisableTestOllama = process.env.CLOSEDCLAW_DISABLE_TEST_OLLAMA;
      ...
      try {
        vi.resetModules();
        const { ensureClosedClawModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureClosedClawModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
        ...
        if (previousDisableTestOllama === undefined) {
          delete process.env.CLOSEDCLAW_DISABLE_TEST_OLLAMA;
        } else {
          process.env.CLOSEDCLAW_DISABLE_TEST_OLLAMA = previousDisableTestOllama;
        }
      }
    });
  });
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureClosedClawModelsJson } = await import("./models-config.js");
      const { resolveClosedClawAgentDir } = await import("./agent-paths.js");

      await ensureClosedClawModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveClosedClawAgentDir(), "models.json");
  ...
  it("adds minimax provider when MINIMAX_API_KEY is set", async () => {
    await withTempHome(async () => {
      ...
      try {
        ...
        await ensureClosedClawModelsJson({});
