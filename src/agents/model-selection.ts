...
export function isCliProvider(provider: string, cfg?: ClosedClawConfig): boolean {
...
export function resolveConfiguredModelRef(params: {
  ...
  if (rawModel) {
    ...
    if (!trimmed.includes("/")) {
      ...
      // Default to the configured runtime default provider if no provider is specified,
      // but warn as this is deprecated.
      console.warn(
        `[ClosedClaw] Model "${trimmed}" specified without provider. Falling back to "${params.defaultProvider}/${trimmed}". Please use "${params.defaultProvider}/${trimmed}" in your config.`,
      );
