...
/**
 * Cleanup suspended ClosedClaw CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: ClosedClawConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-cli-images-"));
  const paths: string[] = [];
