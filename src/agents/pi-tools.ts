...
function resolveExecConfig(cfg: ClosedClawConfig | undefined) {
...
export function createClosedClawCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createClosedClawReadTool(freshReadTool)];
    }
