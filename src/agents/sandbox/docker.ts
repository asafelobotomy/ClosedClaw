...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "ClosedClaw.sandbox=1");
  args.push("--label", `ClosedClaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `ClosedClaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `ClosedClaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("ClosedClaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`ClosedClaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`ClosedClaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("ClosedClaw sandbox recreate --all");
}
