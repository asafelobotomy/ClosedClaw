...
const ETC_ClosedClaw_DIR = path.join(ROOT_DIR, "etc", "ClosedClaw");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "ClosedClaw.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcClosedClawPath(...parts: string[]) {
  return path.join(ETC_ClosedClaw_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcClosedClawPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "ClosedClaw.json"))).toEqual({
      shared: true,
    });
  });
});
