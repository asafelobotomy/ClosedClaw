...
import type { ClosedClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: ClosedClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: ClosedClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ClosedClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ClosedClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ClosedClawConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as ClosedClawConfig)),
    ),
  };
}
