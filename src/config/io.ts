...
import type { ClosedClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareClosedClawVersions } from "./version.js";
...
function coerceConfig(value: unknown): ClosedClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ClosedClawConfig;
}
...
function stampConfigVersion(cfg: ClosedClawConfig): ClosedClawConfig {
...
function warnIfConfigFromFuture(cfg: ClosedClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareClosedClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer ClosedClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: ClosedClawConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ClosedClawConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as ClosedClawConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as ClosedClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: ClosedClawConfig) {
    clearConfigCache();
...
// module scope. `ClosedClaw_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ClosedClaw_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.ClosedClaw_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
