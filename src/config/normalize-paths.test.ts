  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.ClosedClaw/logs/ClosedClaw.log" },
        hooks: {
          path: "~/.ClosedClaw/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.ClosedClaw/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".ClosedClaw", "logs", "ClosedClaw.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".ClosedClaw", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".ClosedClaw", "telegram.token"),
      );
