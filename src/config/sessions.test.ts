...
describe("sessions", () => {
  ...
  it("updateLastRoute persists channel and target", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-sessions-"));
  ...
  it("derives session transcripts dir from ClosedClaw_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ClosedClaw_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.ClosedClaw_STATE_DIR = "/custom/state";
    try {
