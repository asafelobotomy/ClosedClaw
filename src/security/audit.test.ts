...
describe("security audit", () => {
  ...
  it("flags Windows ACLs when Users can read the state dir", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-security-audit-win-open-"));
    const stateDir = path.join(tmp, "state");
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-security-audit-discord-"));
    process.env.ClosedClaw_STATE_DIR = tmp;
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "ClosedClaw-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.ClosedClaw_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.ClosedClaw_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-security-audit-discord-open-"));
    process.env.ClosedClaw_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ClosedClawConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.ClosedClaw_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-security-audit-slack-"));
    process.env.ClosedClaw_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ClosedClawConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.ClosedClaw_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-security-audit-slack-open-"));
    process.env.ClosedClaw_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ClosedClawConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.ClosedClaw_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-security-audit-telegram-"));
    process.env.ClosedClaw_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ClosedClawConfig = {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: ClosedClawConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.ClosedClaw_GATEWAY_TOKEN;
    process.env.ClosedClaw_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: ClosedClawConfig = {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.ClosedClaw",
      configPath: "/Users/test/Dropbox/.ClosedClaw/ClosedClaw.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: ClosedClawConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "ClosedClaw.json"),
      });
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.ClosedClaw_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.ClosedClaw_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.ClosedClaw_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.ClosedClaw_GATEWAY_PASSWORD;
      } else {
        process.env.ClosedClaw_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.ClosedClaw_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: ClosedClawConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.ClosedClaw_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: ClosedClawConfig = {
        gateway: {
          mode: "remote",
