...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of ClosedClaw")).toBe("friends-of-ClosedClaw");
  ...
  it("matches ids or names", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "steipete", "Friends of ClosedClaw"],
    ...
    expect(allowListMatches(allow, { name: "friends-of-ClosedClaw" })).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-ClosedClaw" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of ClosedClaw"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-ClosedClaw");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-ClosedClaw": { slug: "friends-of-ClosedClaw" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of ClosedClaw"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["ClosedClaw-dm"],
        channelId: "1",
        channelName: "ClosedClaw DM",
        channelSlug: "ClosedClaw-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
