...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "ClosedClaw_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.ClosedClaw_STATE_DIR = path.join(home, ".ClosedClaw");
    ...
    await fs.mkdir(path.join(home, ".ClosedClaw"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.ClosedClaw${path.sep}media`);
