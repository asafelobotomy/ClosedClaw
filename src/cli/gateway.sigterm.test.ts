...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "ClosedClaw-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      ClosedClaw_NO_RESPAWN: "1",
      ClosedClaw_STATE_DIR: stateDir,
      ClosedClaw_SKIP_CHANNELS: "1",
      ClosedClaw_SKIP_GMAIL_WATCHER: "1",
      ClosedClaw_SKIP_CRON: "1",
      ClosedClaw_SKIP_BROWSER_CONTROL_SERVER: "1",
      ClosedClaw_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "ClosedClaw-entry-bootstrap.mjs");
