...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.ClosedClaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + ClosedClaw hooks")
    ...
    .option("--hook-url <url>", "ClosedClaw hook URL")
    .option("--hook-token <token>", "ClosedClaw hook token")
