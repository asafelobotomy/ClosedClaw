...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    ClosedClaw_STATE_DIR: process.env.ClosedClaw_STATE_DIR,
    ClosedClaw_CONFIG_PATH: process.env.ClosedClaw_CONFIG_PATH,
    ClosedClaw_GATEWAY_PORT: process.env.ClosedClaw_GATEWAY_PORT,
    ClosedClaw_PROFILE: process.env.ClosedClaw_PROFILE,
  };

  beforeEach(() => {
    process.env.ClosedClaw_STATE_DIR = "/tmp/ClosedClaw-cli-state";
    process.env.ClosedClaw_CONFIG_PATH = "/tmp/ClosedClaw-cli-state/ClosedClaw.json";
    delete process.env.ClosedClaw_GATEWAY_PORT;
    delete process.env.ClosedClaw_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.ClosedClaw_STATE_DIR !== undefined) {
      process.env.ClosedClaw_STATE_DIR = originalEnv.ClosedClaw_STATE_DIR;
    } else {
      delete process.env.ClosedClaw_STATE_DIR;
    }

    if (originalEnv.ClosedClaw_CONFIG_PATH !== undefined) {
      process.env.ClosedClaw_CONFIG_PATH = originalEnv.ClosedClaw_CONFIG_PATH;
    } else {
      delete process.env.ClosedClaw_CONFIG_PATH;
    }

    if (originalEnv.ClosedClaw_GATEWAY_PORT !== undefined) {
      process.env.ClosedClaw_GATEWAY_PORT = originalEnv.ClosedClaw_GATEWAY_PORT;
    } else {
      delete process.env.ClosedClaw_GATEWAY_PORT;
    }

    if (originalEnv.ClosedClaw_PROFILE !== undefined) {
      process.env.ClosedClaw_PROFILE = originalEnv.ClosedClaw_PROFILE;
    } else {
      delete process.env.ClosedClaw_PROFILE;
    }
  });
