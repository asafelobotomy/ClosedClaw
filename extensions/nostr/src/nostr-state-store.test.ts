...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.ClosedClaw_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-nostr-"));
  process.env.ClosedClaw_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.ClosedClaw_STATE_DIR?.trim() || env.ClosedClaw_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".ClosedClaw");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.ClosedClaw_STATE_DIR = previous;
    }
