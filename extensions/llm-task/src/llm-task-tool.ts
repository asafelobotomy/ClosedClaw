...
// NOTE: This extension is intended to be bundled with ClosedClaw.
// When running from source (tests/dev), ClosedClaw internals live under src/.
...
export function createLlmTaskTool(api: ClosedClawPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via ClosedClaw.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ClosedClaw-llm-task-"));
