...
`ClosedClaw.invoke --each`) without adding new ClosedClaw code per workflow.
...
This extension depends on ClosedClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** ClosedClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.ClosedClaw/extensions` as a standalone plugin directory.
