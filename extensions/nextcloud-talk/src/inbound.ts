...
export async function handleNextcloudTalkInbound(params: {
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as ClosedClawConfig,
    surface: CHANNEL_ID,
  });
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as ClosedClawConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as ClosedClawConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as ClosedClawConfig);
