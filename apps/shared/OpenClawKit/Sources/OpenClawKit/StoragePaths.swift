...
public enum ClosedClawNodeStorage {
    public static func appSupportDir() throws -> URL {
        let base = FileManager().urls(for: .applicationSupportDirectory, in: .userDomainMask).first
        guard let base else {
            throw NSError(domain: "ClosedClawNodeStorage", code: 1, userInfo: [
                NSLocalizedDescriptionKey: "Application Support directory unavailable",
            ])
        }
        return base.appendingPathComponent("ClosedClaw", isDirectory: true)
    }
    ...
    public static func cachesDir() throws -> URL {
        ...
        guard let base else {
            throw NSError(domain: "ClosedClawNodeStorage", code: 2, userInfo: [
