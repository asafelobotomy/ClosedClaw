...
@MainActor
struct ClosedClawChatComposer: View {
    @Bindable var viewModel: ClosedClawChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ClosedClawChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ClosedClawChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ClosedClawPendingAttachment.id)
                { (att: ClosedClawPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ClosedClawPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(ClosedClawChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ClosedClawChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ClosedClawChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ClosedClawChatTheme.subtleCard)
        .clipShape(Capsule())
    }
