...
import Foundation
import Testing
@testable import ClosedClawChatUI
...
private final class TestChatTransport: @unchecked Sendable, ClosedClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ClosedClawChatHistoryPayload]
    private let sessionsResponses: [ClosedClawChatSessionsListResponse]

    private let stream: AsyncStream<ClosedClawChatTransportEvent>
    private let continuation: AsyncStream<ClosedClawChatTransportEvent>.Continuation

    init(
        historyResponses: [ClosedClawChatHistoryPayload],
        sessionsResponses: [ClosedClawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<ClosedClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ClosedClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ClosedClawChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [ClosedClawChatAttachmentPayload]) async throws -> ClosedClawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return ClosedClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ClosedClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ClosedClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ClosedClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = ClosedClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = ClosedClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { ClosedClawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = ClosedClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = ClosedClawChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = ClosedClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { ClosedClawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
