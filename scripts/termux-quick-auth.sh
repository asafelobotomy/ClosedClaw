...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/ClosedClaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "ClosedClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/ClosedClaw/scripts/mobile-reauth.sh'" --id ClosedClaw-auth
        ;;
    *)
