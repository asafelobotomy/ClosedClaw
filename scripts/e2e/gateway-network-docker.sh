...
IMAGE_NAME="ClosedClaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="ClosedClaw-net-e2e-$$"
GW_NAME="ClosedClaw-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "ClosedClaw_GATEWAY_TOKEN=$TOKEN" \
	  -e "ClosedClaw_SKIP_CHANNELS=1" \
	  -e "ClosedClaw_SKIP_GMAIL_WATCHER=1" \
	  -e "ClosedClaw_SKIP_CRON=1" \
	  -e "ClosedClaw_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
