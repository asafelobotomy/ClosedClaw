...
#
# Suggested cron: */30 * * * * /home/admin/ClosedClaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send ClosedClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., ClosedClaw-alerts)
...
STATE_FILE="$HOME/.ClosedClaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via ClosedClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use ClosedClaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via ClosedClaw to $NOTIFY_PHONE..."
            ClosedClaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! ClosedClaw is down. Run: ssh l36 '~/ClosedClaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
