#!/bin/bash
# Setup ClosedClaw Auth Management System
...
echo "=== ClosedClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'ClosedClaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  ClosedClaw message: Send warning via ClosedClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/ClosedClaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/ClosedClaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/ClosedClaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now ClosedClaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status ClosedClaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
