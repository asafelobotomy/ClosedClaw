...
STATUS=$(ssh "$SERVER" '$HOME/ClosedClaw/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/ClosedClaw/scripts/claude-auth-status.sh json' 2>&1)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/ClosedClaw/scripts/mobile-reauth.sh'"
                ;;
            *)
