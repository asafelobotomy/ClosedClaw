...
ZIP=${1:?"Usage: $0 ClosedClaw-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/ClosedClaw/ClosedClaw/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^ClosedClaw-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/ClosedClaw/ClosedClaw/releases/download/v${VERSION}/"}
