...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["ClosedClawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["ClosedClaw", "pollId"]) ??
    readNestedString(value, ["ClosedClaw", "poll", "id"]) ??
    readNestedString(value, ["data", "ClosedClawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "ClosedClaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          ClosedClawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "ClosedClaw poll vote",
          displayText: "Vote recorded",
          value: { ClosedClawPollId: pollId, pollId },
        },
      },
    ],
  };
