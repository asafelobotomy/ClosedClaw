# 🦞 ClosedClaw — Personal AI Assistant
...
**ClosedClaw** is a _personal AI assistant_ you run on your own devices.
...
# Send a message
ClosedClaw message send --to +1234567890 --message "Hello from ClosedClaw"
...
pnpm ClosedClaw onboard --install-daemon

# Dev loop (auto-reload on TS changes)
pnpm gateway:watch
```

Note: `pnpm ClosedClaw ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `ClosedClaw` binary.
...
- Approve with: `ClosedClaw pairing approve <channel> <code>` (then the sender is added to a local allowlist store).
- Public inbound DMs require an explicit opt-in: set `dmPolicy="open"` and include `"*"` in the channel allowlist (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`).

Run `ClosedClaw doctor` to surface risky/misconfigured DM policies.
...
ClosedClaw can auto-configure Tailscale **Serve** (tailnet-only) or **Funnel** (public) while the Gateway stays bound to loopback. Configure `gateway.tailscale.mode`:
...
- `gateway.bind` must stay `loopback` when Serve/Funnel is enabled (ClosedClaw enforces this).
...
- Workspace root: `~/.ClosedClaw/workspace` (configurable via `agents.defaults.workspace`).
- Injected prompt files: `AGENTS.md`, `SOUL.md`, `TOOLS.md`.
- Skills: `~/.ClosedClaw/workspace/skills/<skill>/SKILL.md`.

## Configuration

Minimal `~/.ClosedClaw/ClosedClaw.json` (model + defaults):
...
ClosedClaw was built for **Molty**, a space lobster AI assistant. 🦞
by Peter Steinberger and the community.

- [ClosedClaw.ai](https://ClosedClaw.ai)
- [soul.md](https://soul.md)
- [steipete.me](https://steipete.me)
- [@ClosedClaw](https://x.com/ClosedClaw)
