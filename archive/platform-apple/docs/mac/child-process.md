...
service via the external `ClosedClaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`ClosedClaw_PROFILE`; legacy `com.ClosedClaw.*` is supported).
...
- Writes `~/.ClosedClaw/disable-launchagent`.
...
`--attach-only` (or `--no-launchd`). This sets `~/.ClosedClaw/disable-launchagent`,
