...
async function main() {
    if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
        // Match `ClosedClaw --version` behavior for Swift env/version checks.
    ...
    const bindRaw = argValue(args, "--bind") ??
        process.env.ClosedClaw_GATEWAY_BIND ??
...
void main().catch((err) => {
    console.error("[ClosedClaw] Gateway daemon failed:", err instanceof Error ? (err.stack ?? err.message) : err);
    process.exit(1);
});
