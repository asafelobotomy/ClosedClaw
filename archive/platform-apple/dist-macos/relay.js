...
const BUNDLED_VERSION = (typeof __ClosedClaw_VERSION__ === "string" && __ClosedClaw_VERSION__) ||
    process.env.ClosedClaw_BUNDLED_VERSION ||
    "0.0.0";
...
async function main() {
    ...
    const { ensureClosedClawCliOnPath } = await import("../infra/path-env.js");
    ...
    process.on("uncaughtException", (error) => {
        console.error("[ClosedClaw] Uncaught exception:", formatUncaughtError(error));
        process.exit(1);
    });
    await program.parseAsync(process.argv);
}
void main().catch((err) => {
    console.error("[ClosedClaw] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
    process.exit(1);
});
