...
private let chatUILogger = Logger(subsystem: "ai.ClosedClaw", category: "ClosedClawChatUI")

@MainActor
@Observable
public final class ClosedClawChatViewModel {
    public private(set) var messages: [ClosedClawChatMessage] = []
    ...
    public var attachments: [ClosedClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ClosedClawChatPendingToolCall] = []
    public private(set) var sessions: [ClosedClawChatSessionEntry] = []
    private let transport: any ClosedClawChatTransport
    ...
    private var pendingToolCallsById: [String: ClosedClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any ClosedClawChatTransport) {
    ...
    public var sessionChoices: [ClosedClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [ClosedClawChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [ClosedClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ClosedClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ClosedClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ClosedClawChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [ClosedClawChatMessage]) -> [ClosedClawChatMessage] {
        var result: [ClosedClawChatMessage] = []
    ...
    private static func dedupeKey(for message: ClosedClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ClosedClawChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> ClosedClawChatAttachmentPayload in
            ClosedClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ClosedClawChatSessionEntry {
        ClosedClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: ClosedClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ClosedClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ClosedClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ClosedClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ClosedClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
