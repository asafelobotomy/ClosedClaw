...
describe("web auto-reply", () => {
  ...
  it("stops after hitting max reconnect attempts", { timeout: TIMEOUT_TEST_SUITE_LONG_MS }, async () => {
    const closeResolvers: Array<() => void> = [];
    const sleep = vi.fn(async () => {});
    const listenerFactory = vi.fn(async () => {
  ...
  it("processes inbound messages without batching and preserves timestamps", async () => {
    ...
    try {
      ...
      expect(firstArgs.Body).toMatch(
        new RegExp(`\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${firstPattern}\\] \\[ClosedClaw\\] first`),
      );
      expect(firstArgs.Body).not.toContain("second");
      expect(secondArgs.Body).toMatch(
        new RegExp(`\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${secondPattern}\\] \\[ClosedClaw\\] second`),
      );
