...
describe("directory (config-backed)", () => {
  it("lists Slack peers/groups from config", async () => {
    ...
    expect(peers?.map((e) => e.id).toSorted((a, b) => a.localeCompare(b))).toEqual([
      "user:u123",
      "user:u234",
      "user:u777",
      "user:u999",
  ...
  it("lists Discord peers/groups from config (numeric ids only)", async () => {
    ...
    expect(peers?.map((e) => e.id).toSorted((a, b) => a.localeCompare(b))).toEqual(["user:111", "user:12345", "user:222"]);
    ...
    expect(groups?.map((e) => e.id).toSorted((a, b) => a.localeCompare(b))).toEqual(["channel:555", "channel:666"]);
  });
  ...
  it("lists Telegram peers/groups from config", async () => {
    ...
    expect(peers?.map((e) => e.id).toSorted((a, b) => a.localeCompare(b))).toEqual(["123", "456", "@alice", "@bob"]);
