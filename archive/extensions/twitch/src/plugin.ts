/**
 * Twitch channel plugin for ClosedClaw.
 *
...
import { buildChannelConfigSchema } from "ClosedClaw/plugin-sdk";
import type {
 ...
 * for ClosedClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: ClosedClawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: ClosedClawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: ClosedClawConfig): boolean => {
