...
ClosedClaw extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
ClosedClaw channels login --channel zalouser
...
```bash
ClosedClaw message send --channel zalouser --target <threadId> --message "Hello from ClosedClaw!"
```
...
```bash
ClosedClaw channels login --channel zalouser              # Login via QR
ClosedClaw channels login --channel zalouser --account work
ClosedClaw channels status --probe
ClosedClaw channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
ClosedClaw directory self --channel zalouser
ClosedClaw directory peers list --channel zalouser --query "name"
ClosedClaw directory groups list --channel zalouser --query "work"
ClosedClaw directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
ClosedClaw message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
ClosedClaw message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `ClosedClaw channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
ClosedClaw channels login --channel zalouser --account work
ClosedClaw message send --channel zalouser --account work --target <id> --message "Hello"
