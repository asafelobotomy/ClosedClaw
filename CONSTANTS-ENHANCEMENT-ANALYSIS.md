 ...
 */
export function buildGatewayHttpUrl(port: number = DEFAULT_GATEWAY_PORT, host: string = LOCALHOST_IPV4): string {
 ...
 */
export function buildGatewayWsUrl(port: number = DEFAULT_GATEWAY_PORT, host: string = LOCALHOST_IPV4): string {
  return `ws://${host}:${port}`;
}
 ...
 */
export function buildSignalUrl(port: number = DEFAULT_SIGNAL_PORT, host: string = LOCALHOST_IPV4): string {
  return `http://${host}:${port}`;
}
...
export function getTestGatewayHttpUrl(): string {
  const port = Number.parseInt(process.env.ClosedClaw_GATEWAY_PORT ?? `${DEFAULT_GATEWAY_PORT}`, 10);
...
const port = Number.parseInt(process.env[ENV_CLOSEDCLAW_GATEWAY_PORT] ?? `${DEFAULT_GATEWAY_PORT}`, 10);
