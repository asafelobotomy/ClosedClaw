...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/ClosedClaw/tools/dev/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "ClosedClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/ClosedClaw/tools/platform/mobile/mobile-reauth.sh'" --id ClosedClaw-auth
        ;;
    *)
