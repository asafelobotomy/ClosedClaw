...
# ClosedClaw Auth Widget for Termux
...
# Check auth status
termux-toast "Checking ClosedClaw auth..."

STATUS=$(ssh "$SERVER" '$HOME/ClosedClaw/tools/dev/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/ClosedClaw/tools/dev/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|ClosedClaw_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/ClosedClaw/tools/platform/mobile/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|ClosedClaw_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: ClosedClaw won't work until re-auth"
                ;;
        esac
        ;;
