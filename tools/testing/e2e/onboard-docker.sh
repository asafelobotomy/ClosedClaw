...
IMAGE_NAME="ClosedClaw-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/ClosedClaw-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/ClosedClaw-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/ClosedClaw-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/ClosedClaw-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/ClosedClaw-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.ClosedClaw/workspace"
    config_path="$HOME/.ClosedClaw/ClosedClaw.json"
    sessions_dir="$HOME/.ClosedClaw/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    export HOME="$home_dir"
    mkdir -p "$HOME/.ClosedClaw"
    # Seed a remote config to exercise reset path.
    cat > "$HOME/.ClosedClaw/ClosedClaw.json" <<'"'"'JSON'"'"'
{
