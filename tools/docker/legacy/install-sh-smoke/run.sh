...
INSTALL_URL="${ClosedClaw_INSTALL_URL:-https://ClosedClaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${ClosedClaw_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ClosedClaw_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="ClosedClaw"
PACKAGE_NAME="${ClosedClaw_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ClosedClaw_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ClosedClaw_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${ClosedClaw_INSTALL_LATEST_OUT:-}"
fi
