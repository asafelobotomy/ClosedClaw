...
BASE_IMAGE="${BASE_IMAGE:-closedclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-closedclaw-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=ClosedClaw.sandbox=1)
NOTE
