...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ClosedClaw
...
Running ClosedClaw via Node with `tsx` fails at startup with:

```
[ClosedClaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `ClosedClaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch ClosedClaw.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node ClosedClaw.mjs status` works on Node 25.
