...
```bash
ClosedClaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `ClosedClaw tui --deliver`
...
- Check the Gateway logs: `ClosedClaw logs --follow`.
- Confirm the agent can run: `ClosedClaw status` and `ClosedClaw models status`.
...
- No agents in picker: check `ClosedClaw agents list` and your routing config.
