...
- Jobs persist under `~/.ClosedClaw/cron/` so restarts don’t lose schedules.
...
ClosedClaw cron list
ClosedClaw cron run <job-id> --force
ClosedClaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.ClosedClaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `ClosedClaw cron add/edit` or the cron
...
- Job store: `~/.ClosedClaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.ClosedClaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.ClosedClaw/cron/jobs.json",
...
ClosedClaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
ClosedClaw cron edit <jobId> --agent ops
ClosedClaw cron edit <jobId> --clear-agent
```
...
```bash
ClosedClaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`ClosedClaw system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `ClosedClaw_SKIP_CRON`.
