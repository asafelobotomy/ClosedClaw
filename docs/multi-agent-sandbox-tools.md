...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `ClosedClaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `ClosedClaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
4. **Monitor logs:**
   ```exec
   tail -f "${ClosedClaw_STATE_DIR:-$HOME/.ClosedClaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
