---
summary: "All configuration options for ~/.ClosedClaw/ClosedClaw.json with examples"
read_when:
...
ClosedClaw reads an optional **JSON5** config from `~/.ClosedClaw/ClosedClaw.json` (comments + trailing commas allowed).

If the file is missing, ClosedClaw uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.ClosedClaw/workspace`). You usually only need a config to:
...
ClosedClaw only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `ClosedClaw doctor`, `ClosedClaw logs`, `ClosedClaw health`, `ClosedClaw status`, `ClosedClaw service`, `ClosedClaw help`).
- Run `ClosedClaw doctor` to see the exact issues.
- Run `ClosedClaw doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `ClosedClaw config set`. Keep a backup of `~/.ClosedClaw/ClosedClaw.json`.
...
```bash
ClosedClaw gateway call config.get --params '{}' # capture payload.hash
ClosedClaw gateway call config.apply --params '{
...
```bash
ClosedClaw gateway call config.get --params '{}' # capture payload.hash
ClosedClaw gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.ClosedClaw/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@ClosedClaw", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.ClosedClaw/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.ClosedClaw/workspace" }],
}
```
...
{ "$include": "/etc/ClosedClaw/base.json5" } // absolute
...
```json5
// ~/.ClosedClaw/clients/mueller/agents.json5
[
...
```json5
// ~/.ClosedClaw/clients/mueller/broadcast.json5
{
...
ClosedClaw reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, ClosedClaw runs your login shell and imports only the missing expected keys (never overrides).
...
- `ClosedClaw_LOAD_SHELL_ENV=1`
- `ClosedClaw_SHELL_ENV_TIMEOUT_MS=15000`
...
```json5
{
  ...
  gateway: {
    auth: {
      token: "${ClosedClaw_GATEWAY_TOKEN}",
    },
  },
}
```
...
ClosedClaw stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.ClosedClaw/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.ClosedClaw/credentials/oauth.json` (or `$ClosedClaw_STATE_DIR/credentials/oauth.json`)
...
- `~/.ClosedClaw/agent/*` (migrated by `ClosedClaw doctor` into `~/.ClosedClaw/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `ClosedClaw_OAUTH_DIR`
- Agent dir (default agent root override): `ClosedClaw_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, ClosedClaw imports `oauth.json` entries into `auth-profiles.json`.
...
If set, ClosedClaw derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/ClosedClaw/ClosedClaw-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/ClosedClaw/ClosedClaw.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/ClosedClaw/ClosedClaw.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.ClosedClaw/credentials/whatsapp/biz
          // authDir: "~/.ClosedClaw/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `ClosedClaw doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@ClosedClaw", "ClosedClaw"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@ClosedClaw"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.ClosedClaw/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.ClosedClaw/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.ClosedClaw/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.ClosedClaw/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.ClosedClaw/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.ClosedClaw/workspace-home" },
      { id: "work", workspace: "~/.ClosedClaw/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `ClosedClaw.json`).
...
ClosedClaw starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["ClosedClaw-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-ClosedClaw",
...
ClosedClaw starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `ClosedClaw plugins install @ClosedClaw/mattermost` (or `./extensions/mattermost` from a git checkout).
...
ClosedClaw starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
ClosedClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, ClosedClaw
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/ClosedClaw" } },
}
```
...
When a file exceeds this limit, ClosedClaw logs a warning and injects a truncated
...
message envelopes). If unset, ClosedClaw uses the host timezone at runtime.
...
`[ClosedClaw]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[ClosedClaw]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[ClosedClaw]"`, ClosedClaw will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, ClosedClaw generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.ClosedClaw/settings/tts.json",
      elevenlabs: {
...
ClosedClaw also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, ClosedClaw currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `ClosedClaw configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:ClosedClaw`: all built-in ClosedClaw tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.ClosedClaw/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "closedclaw-sandbox-browser:bookworm-slim",
          containerPrefix: "closedclaw-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
ClosedClaw uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.ClosedClaw/agents/<agentId>/agent/models.json` or by defining the same schema inside your
ClosedClaw config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, ClosedClaw writes/merges a `models.json` into
`~/.ClosedClaw/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. ClosedClaw uses
...
- Shortcut: `ClosedClaw onboard --auth-choice opencode-zen`.
...
Shortcut: `ClosedClaw onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `ClosedClaw onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `ClosedClaw onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `ClosedClaw onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `ClosedClaw onboard --auth-choice minimax-api`.
...
- Override the agent config root with `ClosedClaw_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.ClosedClaw/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.ClosedClaw/agents/<agentId>/sessions/sessions.json
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, ClosedClaw stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.ClosedClaw/skills` (workspace skills
...
from `~/.ClosedClaw/extensions`, `<workspace>/.ClosedClaw/extensions`, plus any
...
ClosedClaw can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for ClosedClaw and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (ClosedClaw.app menubar, or `ClosedClaw gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      ClosedClaw: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/ClosedClaw" }
...
- Examples: `"/ui"`, `"/ClosedClaw"`, `"/apps/ClosedClaw"`.
...
- When a connection comes from one of these IPs, ClosedClaw uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `ClosedClaw gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `ClosedClaw_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `ClosedClaw_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. ClosedClaw
...
- ClosedClaw.app watches `~/.ClosedClaw/ClosedClaw.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.ClosedClaw/ClosedClaw.json` (or `ClosedClaw_CONFIG_PATH`) and applies changes automatically.
...
- `~/.ClosedClaw/ClosedClaw.json` (or `ClosedClaw_CONFIG_PATH`)
...
- `ClosedClaw_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `ClosedClaw --dev …` → uses `~/.ClosedClaw-dev` + shifts ports from base `19001`
- `ClosedClaw --profile <name> …` → uses `~/.ClosedClaw-<name>` (port via config/env/flags)
...
```bash
ClosedClaw_CONFIG_PATH=~/.ClosedClaw/a.json \
ClosedClaw_STATE_DIR=~/.ClosedClaw-a \
ClosedClaw gateway --port 19001
```
...
```json5
{
  hooks: {
    ...
    path: "/hooks",
    presets: ["gmail"],
    transformsDir: "~/.ClosedClaw/hooks",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Gmail helper config (used by `ClosedClaw webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "ClosedClaw@gmail.com",
...
- Set `ClosedClaw_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, ClosedClaw defaults `serve.path` to `/` so
...
Default root: `~/.ClosedClaw/workspace/canvas`  
Default port: `18793` (chosen to avoid the ClosedClaw browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__ClosedClaw__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__ClosedClaw__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.ClosedClaw/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- env: `ClosedClaw_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.ClosedClaw/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.ClosedClaw/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.ClosedClaw/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_ClosedClaw-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `ClosedClaw` (advertises `ClosedClaw.local`). Override with `ClosedClaw_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_ClosedClaw-gw._tcp` under `~/.ClosedClaw/dns/` using the configured discovery domain (example: `ClosedClaw.internal.`).
