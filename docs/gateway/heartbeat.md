...
- During heartbeat runs, ClosedClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `ClosedClaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, ClosedClaw skips the heartbeat run entirely (no model call).
...
```bash
ClosedClaw system event --text "Check for urgent follow-ups" --mode now
```
