...
- Replaces the legacy `gateway` command. CLI entry point: `ClosedClaw gateway`.
...
- Config hot reload watches `~/.ClosedClaw/ClosedClaw.json` (or `ClosedClaw_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__ClosedClaw__/canvas/` from `~/.ClosedClaw/workspace/canvas`. Disable with `canvasHost.enabled=false` or `ClosedClaw_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `ClosedClaw_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `ClosedClaw_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.ClosedClaw.*` may still exist)
- Linux: `ClosedClaw-gateway-<profile>.service`
- Windows: `ClosedClaw Gateway (<profile>)`

Install metadata is embedded in the service config:

- `ClosedClaw_SERVICE_MARKER=ClosedClaw`
- `ClosedClaw_SERVICE_KIND=gateway`
...
```bash
ClosedClaw --dev setup
ClosedClaw --dev gateway --allow-unconfigured
# then target the dev instance:
ClosedClaw --dev status
ClosedClaw --dev health
```

Defaults (can be overridden via env/flags/config):

- `ClosedClaw_STATE_DIR=~/.ClosedClaw-dev`
- `ClosedClaw_CONFIG_PATH=~/.ClosedClaw-dev/ClosedClaw.json`
- `ClosedClaw_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.ClosedClaw/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `ClosedClaw_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `ClosedClaw_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `ClosedClaw_CONFIG_PATH`
- unique `ClosedClaw_STATE_DIR`
...
```bash
ClosedClaw --profile main gateway install
ClosedClaw --profile rescue gateway install
```

Example:

```bash
ClosedClaw_CONFIG_PATH=~/.ClosedClaw/a.json ClosedClaw_STATE_DIR=~/.ClosedClaw-a ClosedClaw gateway --port 19001
ClosedClaw_CONFIG_PATH=~/.ClosedClaw/b.json ClosedClaw_STATE_DIR=~/.ClosedClaw-b ClosedClaw gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `ClosedClaw health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `ClosedClaw`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `ClosedClaw gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.ClosedClaw.*` is cleaned up).
  - `ClosedClaw doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are ClosedClaw profile services.
  ...
  - Cleanup: `ClosedClaw gateway uninstall` (current service) and `ClosedClaw doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `ClosedClaw gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- ClosedClaw.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.ClosedClaw.*` labels still unload cleanly).
- To stop it cleanly, use `ClosedClaw gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `ClosedClaw gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `ClosedClaw gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

ClosedClaw installs a **systemd user service** by default on Linux/WSL2. We
...
`ClosedClaw gateway install` writes the user unit. `ClosedClaw doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/ClosedClaw-gateway[-<profile>].service`:

```
[Unit]
Description=ClosedClaw Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/ClosedClaw gateway --port 18789
Restart=always
RestartSec=5
Environment=ClosedClaw_GATEWAY_TOKEN=
...
```
systemctl --user enable --now ClosedClaw-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/ClosedClaw-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now ClosedClaw-gateway[-<profile>].service
```
...
- `ClosedClaw gateway health|status` — request health/status over the Gateway WS.
- `ClosedClaw message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `ClosedClaw agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `ClosedClaw gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `ClosedClaw gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `ClosedClaw gateway` and the legacy TCP control port.
