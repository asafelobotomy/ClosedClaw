...
- 所有插件都必须提供插件清单（`ClosedClaw.plugin.json`）。

## 插件 schema 强制执行

- 每个插件为其配置提供严格的 JSON Schema（内联在清单中）。
- 插件加载流程：
  1. 解析插件清单 + schema（`ClosedClaw.plugin.json`）。
  2. 根据 schema 验证配置。
...
其他所有命令必须直接失败并提示："Config invalid. Run `ClosedClaw doctor --fix`."
