...
read_when:
  - 设置 Mattermost
  - 调试 Mattermost 路由
summary: Mattermost 机器人设置和 ClosedClaw 配置
title: Mattermost
x-i18n:
...
如果你在配置/新手引导期间选择了 Mattermost 并检测到 git 检出，ClosedClaw 会自动提供本地安装路径。
...
在 `ClosedClaw message send` 或定时任务/webhook 中使用以下目标格式：
