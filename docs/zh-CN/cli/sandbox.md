...
ClosedClaw 可以在隔离的 Docker 容器中运行智能体以确保安全性。`sandbox` 命令帮助你管理这些容器，尤其是在更新或配置变更之后。
...
```bash
ClosedClaw sandbox list
ClosedClaw sandbox list --browser  # 仅列出浏览器容器
ClosedClaw sandbox list --json     # JSON 输出
```
...
```bash
ClosedClaw sandbox recreate --all                # 重新创建所有容器
ClosedClaw sandbox recreate --session main       # 指定会话
ClosedClaw sandbox recreate --agent mybot        # 指定智能体
ClosedClaw sandbox recreate --browser            # 仅浏览器容器
ClosedClaw sandbox recreate --all --force        # 跳过确认提示
```
...
**解决方案：** 使用 `ClosedClaw sandbox recreate` 强制移除旧容器。它们会在下次需要时自动使用当前设置重新创建。

提示：建议使用 `ClosedClaw sandbox recreate` 而非手动执行 `docker rm`。它使用 Gateway网关的容器命名规则，避免在作用域/会话键发生变化时出现不匹配问题。

## 配置

沙箱设置位于 `~/.ClosedClaw/ClosedClaw.json` 中的 `agents.defaults.sandbox` 下（按智能体覆盖的配置放在 `agents.list[].sandbox` 中）：
