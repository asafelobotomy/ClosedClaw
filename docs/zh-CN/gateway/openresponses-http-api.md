...
ClosedClaw 的 Gateway网关可以提供一个兼容 OpenResponses 的 `POST /v1/responses` 端点。
...
底层实现中，请求作为普通的 Gateway网关智能体运行来执行（与 `ClosedClaw agent` 相同的代码路径），因此路由/权限/配置与你的 Gateway网关一致。
...
或通过头指定特定的 ClosedClaw 智能体：

- `x-ClosedClaw-agent-id: <agentId>`（默认：`main`）

高级用法：

- `x-ClosedClaw-session-key: <sessionKey>` 完全控制会话路由。
