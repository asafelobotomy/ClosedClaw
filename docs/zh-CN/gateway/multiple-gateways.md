...
summary: 在同一主机上运行多个 ClosedClaw Gateway网关（隔离、端口和配置文件）
title: 多个 Gateway网关
x-i18n:
...
- `ClosedClaw_CONFIG_PATH` — 每个实例独立的配置文件
- `ClosedClaw_STATE_DIR` — 每个实例独立的会话、凭据、缓存
...
配置文件会自动限定 `ClosedClaw_STATE_DIR` + `ClosedClaw_CONFIG_PATH` 的作用域，并为服务名称添加后缀。
