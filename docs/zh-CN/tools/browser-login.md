...
当网站需要登录时，请在**宿主**浏览器配置文件（ClosedClaw 浏览器）中**手动登录**。
...
ClosedClaw 控制一个**专用 Chrome 配置文件**（名为 `ClosedClaw`，橙色调界面）。它与你的日常浏览器配置文件是分开的。
...
如果你有多个配置文件，传入 `--browser-profile <name>`（默认值为 `ClosedClaw`）。
