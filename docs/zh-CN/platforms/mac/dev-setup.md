...
read_when:
  - 设置 macOS 开发环境
summary: 面向 ClosedClaw macOS 应用开发者的设置指南
title: macOS 开发环境设置
x-i18n:
...
本指南介绍从源代码构建和运行 ClosedClaw macOS 应用的必要步骤。
...
要构建 macOS 应用并将其打包为 `dist/ClosedClaw.app`，请运行：
...
macOS 应用需要全局安装 `ClosedClaw` CLI 来管理后台任务。
...
1. 重置 TCC 权限：
   ...
   ```
2. 如果仍然无效，在 [`scripts/package-mac-app.sh`](https://github.com/ClosedClaw/ClosedClaw/blob/main/scripts/package-mac-app.sh) 中临时更改 `BUNDLE_ID`，以强制 macOS 使用"全新状态"。
