...
Deploy ClosedClaw on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/ClosedClaw` — Control UI
...
- `ClosedClaw_STATE_DIR=/data/.ClosedClaw` (recommended)
- `ClosedClaw_WORKSPACE_DIR=/data/workspace` (recommended)
- `ClosedClaw_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your ClosedClaw state + workspace so you can migrate to another host without losing config or memory.
