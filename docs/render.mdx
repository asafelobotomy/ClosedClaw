...
Deploy ClosedClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/ClosedClaw/ClosedClaw"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run ClosedClaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: ClosedClaw_STATE_DIR
        value: /data/.ClosedClaw
      - key: ClosedClaw_WORKSPACE_DIR
        value: /data/workspace
      - key: ClosedClaw_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: ClosedClaw-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/ClosedClaw`.
...
If you use the original ClosedClaw repository, Render will not auto-deploy your ClosedClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For ClosedClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any ClosedClaw host.
