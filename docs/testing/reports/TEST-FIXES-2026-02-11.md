...
const authorized = await resolveCommandAuthorization(
  "/new",
...
// Now:
export async function generateKeyCommand(runtime: RuntimeEnv, params: {
  signerName?: string;
  output?: string;
...
const jsonCall = runtime.log.mock.calls.find(
  ([msg]) => msg.startsWith("{")
);
