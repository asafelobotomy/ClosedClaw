---
summary: "Top-level overview of ClosedClaw, features, and purpose"
read_when:
  - Introducing ClosedClaw to newcomers
title: "ClosedClaw"
---
...
<p align="center">
  ...
  <a href="https://github.com/ClosedClaw/ClosedClaw/releases">Releases</a> ·
  <a href="/">Docs</a> ·
...
- **Guided setup (recommended):** [Wizard](/start/wizard) (`ClosedClaw onboard`)
- **Open the dashboard (local Gateway):** http://127.0.0.1:18789/ (or http://localhost:18789/)

If the Gateway is running on the same computer, that link opens the browser Control UI
immediately. If it fails, start the Gateway first: `ClosedClaw gateway`.
...
Most operations flow through the **Gateway** (`ClosedClaw gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Canvas host**: HTTP file server on `canvasHost.port` (default `18793`), serving `/__ClosedClaw__/canvas/` for node WebViews; see [Gateway configuration](/gateway/configuration) (`canvasHost`).
...
Switching between npm and git installs later is easy: install the other flavor and run `ClosedClaw doctor` to update the gateway service entrypoint.
...
If you don’t have a global install yet, run the onboarding step via `pnpm ClosedClaw ...` from the repo.
...
- Start here:
  ...
  - [ClosedClaw assistant setup](/start/ClosedClaw)
  - [Skills](/tools/skills)
...
**ClosedClaw = CLAW + TARDIS** — because every space lobster needs a time-and-space machine.
