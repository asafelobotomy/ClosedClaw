---
summary: "Use Synthetic's Anthropic-compatible API in ClosedClaw"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. ClosedClaw registers it as the
...
Note: ClosedClaw's Anthropic client appends `/v1` to the base URL, so use
