---
summary: "Use Venice AI privacy-focused models in ClosedClaw"
read_when:
  - You want privacy-focused inference in ClosedClaw
...
```bash
ClosedClaw onboard --auth-choice venice-api-key
```
...
```bash
ClosedClaw chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, ClosedClaw shows all available Venice models. Pick based on your needs:
...
```bash
ClosedClaw models set venice/claude-opus-45
ClosedClaw models set venice/llama-3.3-70b
```

List all available models:

```bash
ClosedClaw models list | grep venice
```

## Configure via `ClosedClaw configure`

1. Run `ClosedClaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
ClosedClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
ClosedClaw chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
ClosedClaw chat --model venice/claude-opus-45

# Use uncensored model
ClosedClaw chat --model venice/venice-uncensored

# Use vision model with image
ClosedClaw chat --model venice/qwen3-vl-235b-a22b

# Use coding model
ClosedClaw chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `ClosedClaw models list` to see currently available models. Some models may be temporarily offline.
