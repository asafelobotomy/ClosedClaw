---
summary: "Use MiniMax M2.1 in ClosedClaw"
read_when:
  - You want MiniMax models in ClosedClaw
...
```bash
ClosedClaw plugins enable minimax-portal-auth  # skip if already loaded.
ClosedClaw gateway restart  # restart if gateway is already running
ClosedClaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/ClosedClaw/ClosedClaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `ClosedClaw.json`:
...
- Use `ClosedClaw models list` and `ClosedClaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `ClosedClaw configure` and selecting **MiniMax M2.1**, or
