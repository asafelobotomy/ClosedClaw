...
read_when:
  - Designing workspace memory (~/.ClosedClaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep ClosedClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.ClosedClaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.ClosedClaw/workspace` as canonical human-readable memory.
...
```
~/.ClosedClaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in ClosedClaw**, but keep a separable core library.

### Why integrate into ClosedClaw?

- ClosedClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `ClosedClaw memory recall "…" --k 25 --since 30d`
  - `ClosedClaw memory reflect --since 7d`
...
Pragmatic take for `~/.ClosedClaw/workspace`:
