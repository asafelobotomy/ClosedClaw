# ClosedClaw ACP Bridge

This document describes how the ClosedClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`ClosedClaw acp` exposes an ACP agent over stdio and forwards prompts to a running
ClosedClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a ClosedClaw Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `ClosedClaw acp` over stdio.

Example config:

```bash
ClosedClaw config set gateway.remote.url wss://gateway-host:18789
ClosedClaw config set gateway.remote.token <token>
```

Example run:

```bash
ClosedClaw acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
ClosedClaw acp --session agent:main:main
ClosedClaw acp --session agent:design:main
ClosedClaw acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “ClosedClaw ACP” to start a thread.

## Execution Model

- ACP client spawns `ClosedClaw acp` and speaks ACP messages over stdio.
...
```bash
ClosedClaw acp --session agent:main:main
ClosedClaw acp --session-label "support inbox"
ClosedClaw acp --reset-session
```
...
`ClosedClaw acp` resolves the Gateway URL and auth from CLI flags or config:
