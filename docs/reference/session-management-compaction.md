...
This document explains how ClosedClaw manages sessions end-to-end:
...
ClosedClaw is designed around a single **Gateway process** that owns session state.
...
ClosedClaw persists sessions in two layers:
...
ClosedClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (ClosedClaw consumes the events, but Pi decides when to compact).
...
ClosedClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, ClosedClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, ClosedClaw leaves it alone.
...
- `ClosedClaw status` (CLI)
- `ClosedClaw sessions` / `sessions --json`
...
ClosedClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- ClosedClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, ClosedClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
ClosedClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but ClosedClaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `ClosedClaw status`.
