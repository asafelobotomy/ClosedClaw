...
ClosedClaw features that can generate provider usage or paid API calls.
...
- `ClosedClaw status --usage` and `ClosedClaw channels list` show provider **usage windows**
...
ClosedClaw can pick up credentials from:
...
- `ClosedClaw status --usage`
- `ClosedClaw models status --json`
...
`ClosedClaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
