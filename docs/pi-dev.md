...
This guide summarizes a sane workflow for working on the pi integration in ClosedClaw.
...
- Trigger the agent directly:
  - `pnpm ClosedClaw agent --message "Hello" --thinking low`
...
State lives under the ClosedClaw state directory. Default is `~/.ClosedClaw`. If `ClosedClaw_STATE_DIR` is set, use that directory instead.

To reset everything:

- `ClosedClaw.json` for config
