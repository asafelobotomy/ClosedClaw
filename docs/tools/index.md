---
summary: "Agent tool surface for ClosedClaw (browser, canvas, nodes, message, cron) replacing legacy `ClosedClaw-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `ClosedClaw-*` skills
title: "Tools"
---

# Tools (ClosedClaw)

ClosedClaw exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `ClosedClaw-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `ClosedClaw.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, ClosedClaw logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`ClosedClaw node run`).
...
- Requires a Brave API key (recommended: `ClosedClaw configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated ClosedClaw-managed browser.
...
- Quick smoke: `ClosedClaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `ClosedClaw gateway` restart in-place)
