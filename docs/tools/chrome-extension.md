---
summary: "Chrome extension: let ClosedClaw drive your existing Chrome tab"
read_when:
...
The ClosedClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate ClosedClaw-managed Chrome profile.
...
ClosedClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the ClosedClaw release (npm package) as static files. There is no separate “build” step.

After upgrading ClosedClaw:

- Re-run `ClosedClaw browser extension install` to refresh the installed files under your ClosedClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

ClosedClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want ClosedClaw to control.
...
- `ON`: attached; ClosedClaw can drive that tab.
...
Debugging: `ClosedClaw sandbox explain`
...
`ClosedClaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `ClosedClaw browser extension install` first to copy the extension to a stable location under your ClosedClaw state directory.
...
- **This is not isolated** like the dedicated ClosedClaw-managed profile.
