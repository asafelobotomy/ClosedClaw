...
- **gateway host** → `ClosedClaw` process on the gateway machine
...
`~/.ClosedClaw/exec-approvals.json` directly.

CLI: `ClosedClaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
