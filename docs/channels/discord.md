...
3. Set the token for ClosedClaw:
...
- Talk to ClosedClaw via Discord DMs or guild channels.
...
3. Configure ClosedClaw with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `ClosedClaw pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running ClosedClaw in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents ClosedClaw needs
...
Discord uses numeric ids everywhere; ClosedClaw config prefers ids.
...
- First: run `ClosedClaw doctor` and `ClosedClaw channels status --probe` (actionable warnings + quick audits).
...
When enabled, ClosedClaw uses the member identity for allowlists and labels the
...
- On startup, ClosedClaw resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:

- The registered commands mirror ClosedClaw’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; ClosedClaw enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`ClosedClaw gateway --force`) after confirming no other processes own the Discord session.
