...
Matrix is an open, decentralized messaging protocol. ClosedClaw connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: `ClosedClaw plugins install @ClosedClaw/matrix`
   - From a local checkout: `ClosedClaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: ClosedClaw calls the same
     login endpoint, stores the access token in `~/.ClosedClaw/credentials/matrix/credentials.json`,
...
- On first connection, ClosedClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  ClosedClaw logs a warning.
...
`~/.ClosedClaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, ClosedClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
