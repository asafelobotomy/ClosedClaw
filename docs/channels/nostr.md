...
read_when:
  - You want ClosedClaw to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables ClosedClaw to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`ClosedClaw onboard`) and `ClosedClaw channels add` list optional channel plugins.
...
```bash
ClosedClaw plugins install @ClosedClaw/nostr
```

Use a local checkout (dev workflows):

```bash
ClosedClaw plugins install --link <path-to-ClosedClaw>/extensions/nostr
```
...
```json
{
  "channels": {
    "nostr": {
      ...
      "profile": {
        "name": "ClosedClaw",
        "displayName": "ClosedClaw",
        "about": "Personal assistant DM bot",
        "picture": "https://example.com/avatar.png",
        "banner": "https://example.com/banner.png",
        "website": "https://example.com",
        "nip05": "ClosedClaw@example.com",
        "lud16": "ClosedClaw@example.com"
      }
    }
  }
}
