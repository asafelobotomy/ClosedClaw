...
read_when:
  - Setting up Twitch chat integration for ClosedClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. ClosedClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
ClosedClaw plugins install @ClosedClaw/twitch
```

Local checkout (when running from a git repo):

```bash
ClosedClaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `ClosedClaw_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "ClosedClaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ClosedClaw_TWITCH_ACCESS_TOKEN env var)
...
```bash
ClosedClaw_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
