...
3. Set tokens for ClosedClaw and start the gateway.
...
7. Slash Commands → create `/ClosedClaw` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
ClosedClaw can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, ClosedClaw falls back to the user token.
...
By default, ClosedClaw replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, ClosedClaw infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `ClosedClaw pairing approve slack <code>`.
...
- On startup, ClosedClaw resolves channel/user names in allowlists to IDs (when tokens allow)
