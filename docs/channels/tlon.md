...
Tlon is a decentralized messenger built on Urbit. ClosedClaw connects to your Urbit ship and can
...
```bash
ClosedClaw plugins install @ClosedClaw/tlon
```

Local checkout (when running from a git repo):

```bash
ClosedClaw plugins install ./extensions/tlon
```
...
Use these with `ClosedClaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, ClosedClaw replies in-thread.
