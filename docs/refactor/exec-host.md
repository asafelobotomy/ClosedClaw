...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.ClosedClaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.ClosedClaw/node.json` (node id + pairing token).
...
Path: `~/.ClosedClaw/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.ClosedClaw/exec-approvals.sock",
...
- Unix socket at `~/.ClosedClaw/exec-approvals.sock` (0600).
