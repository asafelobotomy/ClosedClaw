...
- Publish as `ClosedClaw/plugin-sdk` (or export from core under `ClosedClaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `ClosedClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ClosedClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ClosedClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ClosedClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ClosedClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ClosedClawConfig): string;
  };
};
```
...
- Introduce `ClosedClaw/plugin-sdk`.
- Add `api.runtime` to `ClosedClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `ClosedClawRuntime: ">=2026.2.0"`).
