...
read_when:
  - You want to understand what “context” means in ClosedClaw
...
“Context” is **everything ClosedClaw sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (ClosedClaw-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How ClosedClaw builds the system prompt

The system prompt is **ClosedClaw-owned** and rebuilt each run. It includes:
...
By default, ClosedClaw injects a fixed set of workspace files (if present):
