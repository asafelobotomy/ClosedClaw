...
- `/usage cost` in chats: local cost summary aggregated from ClosedClaw session logs.
- CLI: `ClosedClaw status --usage` prints a full per-provider breakdown.
- CLI: `ClosedClaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
