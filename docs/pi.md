...
This document describes how ClosedClaw integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

ClosedClaw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, ClosedClaw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in ClosedClaw's local TUI mode)                                             |
...
├── pi-tools.ts                    # createClosedClawCodingTools()
...
├── ClosedClaw-tools.ts              # ClosedClaw-specific tools
...
This ensures ClosedClaw's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, ClosedClaw CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
ClosedClaw wraps this with `guardSessionManager()` for tool result safety.
...
ClosedClaw maintains an auth profile store with multiple API keys per provider:
...
ClosedClaw loads custom pi extensions for specialized behavior:
...
ClosedClaw also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | ClosedClaw Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom ClosedClaw tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.ClosedClaw/agents/<agentId>/sessions/` (or `$ClosedClaw_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-tools.create-ClosedClaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-ClosedClaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-ClosedClaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-ClosedClaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
