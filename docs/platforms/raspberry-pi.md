---
summary: "ClosedClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up ClosedClaw on a Raspberry Pi
  - Running ClosedClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# ClosedClaw on Raspberry Pi

## Goal

Run a persistent, always-on ClosedClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most ClosedClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled ClosedClaw

# Enable if not
sudo systemctl enable ClosedClaw

# Start on boot
sudo systemctl start ClosedClaw
```
...
```bash
# Check logs
journalctl -u ClosedClaw --no-pager -n 100

# Common fix: rebuild
cd ~/ClosedClaw  # if using hackable install
npm run build
sudo systemctl restart ClosedClaw
```
