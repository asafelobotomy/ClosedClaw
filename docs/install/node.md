...
read_when:
  - "You installed ClosedClaw but `ClosedClaw` is “command not found”"
...
ClosedClaw’s runtime baseline is **Node 22+**.

If you can run `npm install -g ClosedClaw@latest` but later see `ClosedClaw: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
