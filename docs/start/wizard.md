...
The onboarding wizard is the **recommended** way to set up ClosedClaw on macOS,
...
`ClosedClaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
...
(`web_fetch` works without a key). Easiest path: `ClosedClaw configure --section web`
...
1. **Existing config detection**
   - If `~/.ClosedClaw/ClosedClaw.json` exists, choose **Keep / Modify / Reset**.
   - Re-running the wizard does **not** wipe anything unless you explicitly choose **Reset**
     (or pass `--reset`).
   - If the config is invalid or contains legacy keys, the wizard stops and asks
     you to run `ClosedClaw doctor` before continuing.
...
2. **Model/Auth**
   ...
   - **OpenAI API key**: uses `OPENAI_API_KEY` if present or prompts for a key, then saves it to `~/.ClosedClaw/.env` so launchd can read it.
...
- OAuth credentials live in `~/.ClosedClaw/credentials/oauth.json`; auth profiles live in `~/.ClosedClaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
- More detail: [/concepts/oauth](/concepts/oauth)

3. **Workspace**
   - Default `~/.ClosedClaw/workspace` (configurable).
...
7. **Health check**
   ...
   - Tip: `ClosedClaw status --deep` adds gateway health probes to status output (requires a reachable gateway).
...
Use `ClosedClaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.ClosedClaw/workspace-<agentId>`.
