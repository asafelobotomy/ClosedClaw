...
- Skills: update session-logs paths from .clawdbot to .ClosedClaw. (#4502)
...
- Build: align npm tar override and bin metadata so the `ClosedClaw` CLI entrypoint is preserved in npm publishes.
- Docs: add pi/pi-dev docs and update ClosedClaw branding + install links.
...
- Rebrand: rename the npm package/CLI to `ClosedClaw`, add a `ClosedClaw` compatibility shim, and move extensions to the `@ClosedClaw/*` scope.
...
- macOS: finish ClosedClaw app rename for macOS sources, bundle identifiers, and shared kit paths. (#2844) Thanks @fal3.
...
- Docs: update exe.dev install instructions. (#https://github.com/ClosedClaw/ClosedClaw/pull/3047) Thanks @zackerthescar.
...
- Skills: update session-logs paths to use ~/.ClosedClaw. (#4502) Thanks @bonald.
...
- CLI: auth probe output polish (table output, inline errors, reduced noise, and wrap fixes in `ClosedClaw models status`). (commit da3f2b489, commit 00ae21bed, commit 31e59cd58, commit f7dc27f2d, commit 438e782f8, commit 886752217, commit aabe0bed3, commit 81535d512, commit c63144ab1)
...
- CLI: flatten node service commands under `ClosedClaw node` and remove `service node` docs.
- CLI: move gateway service commands under `ClosedClaw gateway` and add `gateway probe` for reachability.
...
- Build: use tsgo for dev/watch builds by default (opt out with `ClosedClaw_TS_COMPILER=tsc`).
...
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `ClosedClaw doctor --fix` to repair, then update plugins (`ClosedClaw plugins update`) if you use any.
...
- CLI: keep `ClosedClaw logs` output resilient to broken pipes while preserving progress output.
...
- Memory: show total file counts + scan issues in `ClosedClaw memory status`.
...
- Memory: probe sqlite-vec availability in `ClosedClaw memory status`.
...
- **BREAKING:** `ClosedClaw message` and message tool now require `target` (dropping `to`/`channelId` for destinations). (#1034) — thanks @tobalsan.
...
- Directory: unify `ClosedClaw directory` across channels and plugin channels.
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `ClosedClaw.hooks` manifests and `ClosedClaw hooks install/update`.
...
- Plugins: add provider auth registry + `ClosedClaw models auth login` for plugin-driven OAuth/API key flows.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@ClosedClaw/msteams` via `ClosedClaw plugins install @ClosedClaw/msteams`.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.

### Changes

- UI/Apps: move channel/config settings to schema-driven forms and rename Connections → Channels. (#1040) — thanks @thewilloftheshadow.
- CLI: set process titles to `ClosedClaw-<command>` for clearer process listings.
...
- Security: warn on weak model tiers (Haiku, below GPT-5, below Claude 4.5) in `ClosedClaw security audit`.
...
- Auth/Status: keep auth profiles sticky per session (rotate on compaction/new), surface provider usage headers in `/status` and `ClosedClaw models status`, and update docs.
- CLI: add `--json` output for `ClosedClaw daemon` lifecycle/install commands.
...
- Fix: make `ClosedClaw update` auto-update global installs when installed via a package manager.
- Fix: list model picker entries as provider/model pairs for explicit selection. (#970) — thanks @mcinteerj.
- Fix: align OpenAI image-gen defaults with DALL-E 3 standard quality and document output formats. (#880) — thanks @mkbehr.
- Fix: persist `gateway.mode=local` after selecting Local run mode in `ClosedClaw configure`, even if no other sections are chosen.
...
- Security: expanded `ClosedClaw security audit` (+ `--fix`), detect-secrets CI scan, and a `SECURITY.md` reporting policy.
...
- Security: expand `ClosedClaw security audit` checks (model hygiene, config includes, plugin allowlists, exposure matrix) and extend `--fix` to tighten more sensitive state paths.
...
- Browser: add Chrome extension relay takeover mode (toolbar button), plus `ClosedClaw browser extension install/path` and remote browser control (standalone server + token auth).
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.ClosedClaw-dev`).
...
- Memory: new `ClosedClaw memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.ClosedClaw/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Gateway/CLI: add Tailscale binary discovery, custom bind mode, and probe auth retry; add `ClosedClaw dashboard` auto-open flow; default native slash commands to `"auto"` with per-provider overrides. (#740) — thanks @jeffersonwarrior.
...
- Install: run `ClosedClaw doctor --non-interactive` after git installs/updates and nudge daemon restarts when detected.
...
- Gateway/CLI: honor `CLAWDBOT_LAUNCHD_LABEL` / `CLAWDBOT_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `ClosedClaw doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- Plugins: add `ClosedClaw plugins install` (path/tgz/npm), plus `list|info|enable|disable|doctor` UX.
...
- macOS: prompt to install the global `ClosedClaw` CLI when missing in local mode; install via `ClosedClaw.ai/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Installer tests: add root+non-root docker smokes, CI workflow to fetch ClosedClaw.ai scripts and run install sh/cli with onboarding skipped.
...
- Plugins: treat `plugins.load.paths` directory entries as package roots when they contain `package.json` + `ClosedClaw.extensions`; load plugin packages from config dirs; extract archives without system tar.
...
- CLI: `ClosedClaw status` now table-based + shows OS/update/gateway/daemon/agents/sessions; `status --all` adds a full read-only debug report (tables, log tails, Tailscale summary, and scan progress via OSC-9 + spinner).
...
- CLI: add `ClosedClaw update` (safe-ish git checkout update) + `--update` shorthand. (#673) — thanks @fm1randa.
...
- CLI: add `ClosedClaw reset` and `ClosedClaw uninstall` flows (interactive + non-interactive) plus docker cleanup smoke test.
...
- CLI/Gateway: clarify that `ClosedClaw gateway status` reports RPC health (connect + RPC) and shows RPC failures separately from connect failures.
...
- Docs: make `ClosedClaw status` the first diagnostic step, clarify `status --deep` behavior, and document `/whoami` + `/id`.
...
- Sandbox: add `ClosedClaw sandbox explain` (effective policy inspector + fix-it keys); improve “sandbox jail” tool-policy/elevated errors with actionable config key paths; link to docs.
...
- CLI: `ClosedClaw sessions` now includes `elev:*` + `usage:*` flags in the table output.
- CLI/Pairing: accept positional provider for `pairing list|approve` (npm-run compatible); update docs/bot hints.
- Branding: normalize legacy casing/branding to “ClosedClaw” (CLI, status, docs).
- Auto-reply: fix native `/model` not updating the actual chat session (Telegram/Slack/Discord). (#646)
- Doctor: offer to run `ClosedClaw update` first on git installs (keeps doctor output aligned with latest).
- Doctor: avoid false legacy workspace warning when install dir is `~/ClosedClaw`. (#660)
...
- CLI: `ClosedClaw message` now subcommands (`message send|poll|...`) and requires `--provider` unless only one provider configured.
...
- Models/Auth: setup-token + token auth profiles; `ClosedClaw models auth order {get,set,clear}`; per-agent auth candidates in `/model status`; OAuth expiry checks in doctor/status.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.ClosedClaw/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- **SECURITY (update ASAP):** inbound DMs are now **locked down by default** on Telegram/WhatsApp/Signal/iMessage/Discord/Slack.
  ...
  - Approve requests via `ClosedClaw pairing list <provider>` + `ClosedClaw pairing approve <provider> <code>`.
...
- **Gateway/CLI UX:** `ClosedClaw logs`, cron list colors/aliases, docs search, agents list/add/delete flows, status usage snapshots, runtime/auth source display, and `/status`/commands auth unification.
...
- Env: load global `$ClosedClaw_STATE_DIR/.env` (`~/.ClosedClaw/.env`) as a fallback after CWD `.env`.
...
- CLI: run `ClosedClaw agent` via the Gateway by default; use `--local` to force embedded mode.
